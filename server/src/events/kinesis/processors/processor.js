...
function recordProcessor() {
    ...
    var shardId;
    var store = '../stores/' + StreamName + 'Subscribers.json';
    return {
        ...
        processRecords: function (processRecordsInput, completeCallback) {
            ...
            for (var i = 0; i < records.length; ++i) {
                ...
                let response = {};
                response[`${StreamName}`] = JSON.parse(data);
                sequenceNumber = record.sequenceNumber;
                partitionKey = record.partitionKey;
                // console.log(record);
                if (!fs.existsSync(store)) {
                    fs.writeFileSync(store, JSON.stringify(response) + os.EOL);
                } else {
                    fs.appendFile(store, JSON.stringify(response) + os.EOL);
                }
                console.log(util.format('ShardID: %s, Record: %s, SeqenceNumber: %s, PartitionKey:%s', shardId, JSON.stringify(response), sequenceNumber, partitionKey));
            }
            if (!sequenceNumber) {
                completeCallback();
                return;
